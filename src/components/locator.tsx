...
export default async function AgencyLocator() {
  let agencies: Agency[] = [];
  let errorMsg = '';

  try {
    // RLS select policy is public read-only for TO anon, authenticated
    ...
    <div className="relative">
      {errorMsg && (
        <div className="mb-4 text-xs text-emerald-500/80 bg-emerald-500/5 px-3 py-1.5 rounded-lg border border-emerald-500/10 font-mono text-center max-w-lg mx-auto">
          Offline Mode: Utilizing local seed directory.
        </div>
      )}
      <LocatorClient initialAgencies={agencies} />
    </div>
  );
}
