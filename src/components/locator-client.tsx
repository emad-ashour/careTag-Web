...
const enrichAgencies = (agencies: Agency[]): EnrichedAgency[] => {
  return agencies.map((agency) => {
    let rating = 4.7;
    let operatingHours = '8:00 AM - 6:00 PM';
    let promotion = 'Free CareTag Registration With Any Full Oil Change';
    let phone = '+971 4 123 4567';

    if (agency.name.toLowerCase().includes('premium')) {
      rating = 4.9;
      operatingHours = '8:00 AM - 9:00 PM';
      promotion = '10% Off Brake Fluid Replacements';
      phone = '+971 4 999 8888';
    } else if (agency.name.toLowerCase().includes('quick fix')) {
      rating = 4.6;
      operatingHours = '7:30 AM - 6:00 PM';
      promotion = 'Free CareTag Registration With Any Full Oil Change';
...
export default function LocatorClient({ initialAgencies }: LocatorClientProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCity, setSelectedCity] = useState('ALL');
  const [onlyWithPromotions, setOnlyWithPromotions] = useState(false);

  const enrichedAgencies = useMemo(() => enrichAgencies(initialAgencies), [initialAgencies]);
    ...
    <div className="w-full">
      ...
      <div className="glassmorphism rounded-2xl p-6 mb-8 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between relative z-10">
          ...
          <div className="w-full lg:flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by agency name, address, or city..."
              value={searchQuery}
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none self-start lg:self-auto py-1">
            ...
            />
            <span className="text-sm text-slate-300">Promotions Only</span>
          </label>
        </div>
      </div>
      ...
      {filteredAgencies.length > 0 ? (
              ...
              <div>
                ...
                {agency.promotion && (
                  <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3 mb-6 flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-emerald-400 uppercase tracking-wide">Active Campaign</p>
        ...
        <div className="text-center py-16 bg-slate-900/20 border border-slate-900 rounded-2xl">
          ...
          <h4 className="text-slate-300 font-semibold mb-1">No Active Agencies Found</h4>
          <p className="text-slate-500 text-sm max-w-md mx-auto">
            Try adjusting your search terms or choosing a different city filter.
          </p>
        </div>
      )}
    </div>
